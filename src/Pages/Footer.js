import React from 'react';

const Footer = () => {
  return (
    <>
      <div className='footer-container' style={{ paddingBottom: '20px' }}>
        <footer className='bg-primary short-2'>
          <div className='container' style={{ MarginBottom: '20px' }}>
            <div className='row'>
              <div className='col-sm-12 text-center'>
                <i className='icon arrow_down'></i>
                <a
                  href='./Assets/img/Keyur_Adroja_CV.pdf'
                  className='text-white'
                  target='_blank'
                  rel='noopener noreferrer'
                >
                  <span className='text-white'>
                    Download My CV <i className='icon arrow_down'></i>
                  </span>
                </a>
                <br />
                <br />
                <a
                  href='https://www.facebook.com/keyur.adroja'
                  target='_blank'
                  rel='noopener noreferrer'
                >
                  <i
                    className='icon-facebook text-white'
                    style={{ fontSize: '40px' }}
                  ></i>
                </a>

                <a
                  href='https://www.linkedin.com/in/keyur-adroja'
                  target='_blank'
                  rel='noopener noreferrer'
                >
                  {' '}
                  <i
                    className='icon-linkedin text-white'
                    style={{ fontSize: 40 }}
                  ></i>{' '}
                </a>
                <a
                  href='https://github.com/KeyurAdroja'
                  target='_blank'
                  rel='noopener noreferrer'
                >
                  {' '}
                  <i
                    className='lni lni-github text-white'
                    style={{ fontSize: 39, display: 'block' }}
                  ></i>
                </a>
              </div>
            </div>
          </div>
        </footer>
      </div>
    </>
  );
};

export default Footer;
